    ...
    <div class="w-full bg-black_1 h-screen max-h-screen flex gap-3 px-5 pt-3">
      ...
      <div class="flex gap-2">
        <TransitionGroup name="slide">
          <div
            id="card"
            v-for="(bill, index) in state.billInformation"
            :key="index"
          ...
          >
            <div
              class="w-full flex justify-between items-center border-b-[1px] border-b-grey_2"
            >
              <p class="text-[white] text-lg">
                {{ bill?.bill_detail?.bill_title }}
              </p>
              <p class="text-[white] text-normal opacity-[.65]">
                {{ getShortDate(bill?.bill_detail?.bill_date) }}
              </p>
            </div>
            <div class="pt-2 flex flex-col gap-1">
              <p class="text-[white] text-sm">
                ...
                <span class="font-bold">
                  {{ bill?.bill_detail?.items.length }}
                </span>
              </p>
              <p class="text-[white] text-sm">
                Total Spent:
                <span class="font-bold">
                  NRS. {{ getTotalCost(bill?.bill_detail?.items) }}
                </span>
              </p>
            </div>
          </div>
