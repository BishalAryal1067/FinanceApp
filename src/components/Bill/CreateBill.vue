...
const saveBill = async () => {
  const error = await sendBillInformation({
    ...
    bill_detail: JSON.stringify({
      bill_title: billHeading.value,
      bill_date: date.value,
      items: bill,
    }),
  });
...
<template>
  <div class="flex flex-col mt-4 gap-2">
    ...
    <div id="bill-section" class="w-full box-border pr-2 flex-wrap">
      ...
      <div id="bill" class="bg-grey_2 max-w-fit box-border px-3 py-2 rounded">
        ...
        <hr
          class="w-full h-[.2175rem] bg-grey_2 border-dashed stroke-[12px] my-2"
        />
        <div id="items-area" class="max-w-full">
          ...
          <div
            ...
            v-if="bill.length == 0"
          >
            <p class="text-xs my-2 italic text-grey_1 font-medium">
              [Note*]: No item is added. Click on the add item above then enter
              details
            </p>
          </div>
          <!--items-->
          <div
            id="row"
            v-for="(row, index) in bill"
            class="w-full flex justify-start gap-2 items-center mb-2"
          >
            <InputField
              type="text"
              :customStyle="'min-w-[7.5rem] max-w-[12rem]'"
              v-model="row.item"
              placeholder="Item"
            />
            <InputField
              type="text"
              :customStyle="'min-w-[5rem] max-w-[7.5rem]'"
              v-model="row.qty"
  ...
  <!--success Dialog-->
  <BasicModal
    :showModal="showResponseDialog"
    :heading="responseHeading"
    :message="responseMessage"
    @close="showResponseDialog = !showResponseDialog"
  >
    <Button :label="'Ok'" />
  </BasicModal>
</template>
